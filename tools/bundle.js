 ...
 *
 *   holdem-dojo.html  a complete standalone document — doctype, viewport,
 *                     favicon — for double-clicking, GitHub Pages, or a USB
 *                     stick. This is the one a learner keeps.
 *   artifact.html     the same page without the document wrapper, because the
 *                     artifact host supplies its own <head> and <body>.
 *
 * Emitting one file for both was the earlier mistake: dropping the doctype to
 * satisfy the artifact host left the standalone copy rendering in quirks mode.
 */
const OUT_STANDALONE = join(ROOT, 'dist/holdem-dojo.html');
const OUT_ARTIFACT = join(ROOT, 'dist/artifact.html');
...
/** A spade drawn as an inline SVG data URI, so the tab icon needs no network. */
const FAVICON = 'data:image/svg+xml,'
  + encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">'
    + '<rect width="100" height="100" rx="20" fill="#16493c"/>'
    + '<text x="50" y="72" font-size="64" text-anchor="middle" fill="#f7f9fc">&#9824;</text>'
    + '</svg>',
  );
 ...
 * scheduling), src/ui (interface).
 *
 * This file makes no network requests of any kind. Once it is on disk it
 * works with the machine offline, permanently.
 */`;

const body = `<div id="app" class="app">
  <noscript>Hold'em Dojo needs JavaScript. Everything runs on your own machine — nothing is sent anywhere.</noscript>
</div>
...
/** Bare content: the artifact host supplies the document wrapper. */
const artifactHtml = `<title>Hold'em Dojo</title>
...
/** A complete document, for double-clicking or serving from GitHub Pages. */
const standaloneHtml = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
<title>Hold'em Dojo</title>
<meta name="description" content="Learn Texas Hold'em from first principles, fifteen minutes a day.">
...
await mkdir(dirname(OUT_STANDALONE), { recursive: true });
await writeFile(OUT_STANDALONE, standaloneHtml, 'utf8');
await writeFile(OUT_ARTIFACT, artifactHtml, 'utf8');

const kb = (n) => `${(n / 1024).toFixed(0)} KB`;
console.log(`\n  ${sorted.length} modules bundled\n`);
console.log(`    ${relative(ROOT, OUT_STANDALONE)}  ${kb(standaloneHtml.length)}  (standalone, works offline)`);
console.log(`    ${relative(ROOT, OUT_ARTIFACT)}  ${kb(artifactHtml.length)}  (for the artifact host)\n`);

// A missed external reference would silently break offline use, so fail loudly.
const external = [...standaloneHtml.matchAll(/(?:src|href)\s*=\s*["'](?!data:|#)([^"']+)["']/g)];
if (external.length) {
  console.error('  External references found, which would break offline use:');
  for (const m of external) console.error(`    ${m[1]}`);
  process.exit(1);
}
console.log('  No external references — verified offline-safe.\n');
